...
public class BPlusTree {
    ...
    /**
     * 该方法用于获取 B+ 树的根节点的 UID
     * @return
     */
    private long rootUid() {
        bootLock.lock();
        try {
            // 通过 bootDataItem.data() 获取一个 SubArray 类型的数据结构，其中包含了根节点的信息
    ...
    private void updateRootUid(long left, long right, long rightKey) throws Exception {
        ...
        try {
            ...
            // 将新生成的根节点插入到数据库中，并返回新根节点的 UID
            long newRootUid = dm.insert(TransactionManagerImpl.SUPER_XID, rootRaw);
            // 准备更新数据项
            bootDataItem.before();
            // 获取 bootDataItem 的原始数据
            SubArray diRaw = bootDataItem.data();
            // 新根节点的 UID (newRootUid) 转换为字节数组并拷贝到 diRaw.raw 中，更新原始数据的前 8 个字节
