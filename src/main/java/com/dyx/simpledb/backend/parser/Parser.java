...
public class Parser {
    ...
    private static Create parseCreate(CreateTable createTable) {
        ...
        List<String> fieldNames = new ArrayList<>(); // 字段名
        List<String> fieldTypes = new ArrayList<>(); // 字段数据类型
        List<String> indexes = new ArrayList<>(); // 索引列（单列索引）
        List<String> autoIncrement = new ArrayList<>(); // 自增字段
        List<String> notNull = new ArrayList<>(); // 非空字段
        List<String> unique = new ArrayList<>(); // 唯一字段
    ...
    // 该方法用于解析 ABORT SQL 语句
    private static Abort parseAbort() {
        return new Abort();
    }

    // 该方法用于解析 COMMIT SQL 语句
    private static Commit parseCommit() {
        return new Commit();
    }

    // 该方法用于解析 BEGIN SQL 语句，通常用于启动一个新的事务
