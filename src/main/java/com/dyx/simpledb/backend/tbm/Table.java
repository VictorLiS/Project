...
public class Table {
    ...
    private List<Long> parseWhere(Where where, long xid) throws Exception {
        // 如果查询条件 where 为 null，即没有任何条件，那么直接返回所有的 UID
        if (where == null)
            return getAllUid();

        // 解析条件字段
        // 对于查询条件中的两个字段（singleExp1 和 singleExp2），尝试查找它们是否存在索引
    ...
    /**
     * 其作用是根据字段名称（fieldName）查找一个具有索引的 Field 对象。
     * @param fieldName
     * @return
     */
    private Field findIndexedField(String fieldName) {
        return fields.stream()
                .filter(field -> field.fieldName.equals(fieldName) && field.isIndexed()) // field.isIndexed()：检查当前字段是否有索引
                .findFirst()
                .orElse(null); // 如果 findFirst() 没有找到符合条件的元素，Optional 会为空，orElse(null) 将返回 null。
    }

    /**
     * 当`where`条件为空时，直接返回所有UID
     * @return
     * @throws Exception
     */
    ...
    /**
     * 作用是在数据库表中执行全表扫描，并根据给定的条件筛选符合条件的记录（通过 Where 对象）
     * @param where 查询条件
     * @param xid 表示当前事务 ID
     * @return
     * @throws Exception
    ...
    private List<Long> performFullTableScanWithCondition(Where where, long xid) throws Exception {
        ...
        for (Long uid : getAllUid()) {
            // 根据 xid 事务 ID 和 uid 唯一标识符从存储中读取该记录的原始字节数据
            byte[] data = ((TableManagerImpl) tbm).vm.read(xid, uid);
            if (data == null) continue;

            // parseEntry(data) 将原始字节数据（data）转换为一个 Map<String, Object>，
            // 其中 String 表示字段名，Object 表示字段值，解析后的数据存储在 record 中。
    ...
    private boolean satisfiesCondition(Map<String, Object> record, Where where) throws Exception {
        // 代码首先使用 checkSingleCondition(record, where.singleExp1)
        // 方法检查记录是否满足 singleExp1 条件。返回 result1
        ...
        // 再次处理singleExp2的结果
        // 如果 where.singleExp2 不为 null，则继续使用
        // checkSingleCondition(record, where.singleExp2) 来检查第二个条件，返回 result2。
    ...
    /**
     * 旨在将一个字符串值（value）转换为与字段（fieldName）相关的适当类型对象。
     * 比如age 字符串 "30" 转换为 Integer 类型并返回。
     * @param value
     * @param fieldName
     * @return
     */
    ...
    class CalWhereRes {
        long l0, r0, l1, r1;
        boolean single; // 这表明该条件是一个单独的范围查询
    }

    /**
     * 计算一个条件表达式的范围，并返回一个包含计算结果的对象
     * 假设有一个字段 age 和一个条件表达式 age > 30，
     * calExp 方法可能计算出该条件的范围是从 30 到无穷大（left = 30, right = Long.MAX_VALUE）。
     * 在这种情况下，calWhere 方法会返回一个 CalWhereRes 对象，
     * 其 l0 为 30，r0 为 Long.MAX_VALUE，并将 single 标记为 true。
     * @param field 表示字段对象，包含了字段的类型和其它与字段相关的信息
     * @param exp
     * @return
     * @throws Exception
     */
